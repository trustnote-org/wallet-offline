...
function writePublicKeys(pub_key, devicePrivKey, onDone){
	var keys = {
		pub_key: pub_key,
		device_priv_key: devicePrivKey
	};
	fs.writeFile(PUBLIC_KEYS_FILENAME, JSON.stringify(keys, null, '\t'), 'utf8', function(err){
		if (err)
			throw Error("failed to write keys file");
		if (onDone)
			onDone();
	...
	readKeys(function(mnemonic_phrase, passphrase, deviceTempPrivKey, devicePrevTempPrivKey){
		...
		readSingleWallet(function(wallet){
			...
			db.query("SELECT 1 FROM extended_pubkeys WHERE device_address=?", [my_device_address], function(rows){
				...
				//exchange mode write public key
				var strXPubKey = Bitcore.HDPublicKey(xPrivKey.derive("m/44'/0'/0'")).toString();
				var devicePrivKeyBN = xPrivKey.derive("m/1'").privateKey.bn;
				writePublicKeys(strXPubKey, devicePrivKeyBN, function() {
					console.log('Write public key succeed !!!');
					process.exit(0);
				});
			});
		});
